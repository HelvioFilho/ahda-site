window.onload = function () {
	...
	fileChooser.onchange = e => {
		...
		if (fileToUpload) {
			...
			document.getElementsByClassName('file-chooser')[0].value = null;
			previewImg.src = url + '/img/user/generic.jpeg';
		}
	};

	function checkExtension(input, error) {
		var extArquivo = input.split('.').pop();
		var escolha = permissao = "";
		var extPermitidas = ['jpg', 'png', 'jpeg', 'jpe'];
		escolha = "imagem";
		permissao = "<br>jpg, png, jpeg, jpe.";

		if (typeof extPermitidas.find(function (ext) { return extArquivo == ext; }) == 'undefined') {
			error[1].innerHTML = 'A extensão "' + extArquivo + '" não é permitida como ' + escolha + ', tente novamente usando umas das seguintes extensões: ' + permissao;
		} else {
			return true;
		}
		return false;
	...
	document.getElementById('changeAvatar').addEventListener('click', (e) => {
		e.preventDefault();
		let avatar = document.getElementsByClassName('Avatar_error');
		let input = document.getElementsByClassName('file-chooser')[0].value;
		Object.keys(avatar).forEach(item => avatar[item].style.visibility = 'hidden');

		if (input) {
			if (checkExtension(input, avatar)) {
